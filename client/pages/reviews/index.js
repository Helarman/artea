...
export const getServerSideProps = async ({ query: { page = 1} }) => {
    ...
    const [res1, res2, res3] = await Promise.all([
        ...
        fetch(`http://localhost:1337/api/reviews?pagination[page]=${start}&pagination[pageSize]=3`),
...
const ReviewsPage = ({ global, reviews, pageData, page, start }) => {
    ...
    const hidePagination = pageQuantity >= total;
    ...
    const hideFitst = previousPage === 0; //Скрвыем элементы если страница первая
            ...
            <div id="pagination" className={` ${hidePagination ? `${styles.none}` : `${styles.navigation}`}`}>
                ...
                <div className={styles.pageNumbers}>
                    <ul>
                        ...
                        <a className={` ${hidePrevDots ? `${styles.inline}` : `${styles.none}`}`}>...</a>
                        <a className={` ${itsLast && !itsSecond ? `${styles.inline}` : `${styles.none}`}`} onClick={() => router.push(`/reviews?page=${page - 2}#pagination`)}>{page - 2}</a>
                        <a className={` ${hideFitst ? `${styles.none}` : `${styles.inline}`}`} onClick={() => router.push(`/reviews?page=${page - 1}#pagination`)}>{page - 1}</a>
